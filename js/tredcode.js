...
// Heat Map API
function call_Heat_Map_API(script) {
  try {
    $.get(root_1 + `/study-data-clone/Heat%20Map%20${script}`,
      function (data, status) {
        Heat_Map = data.data;
      }
    ).fail(function (response) {
...
function call_Dial_API(script, exp_date, exp_type) {
  $.post(
    root + "/op_dial",
    { script: script, exp: exp_date, exp_type: exp_type },
    function (data, status) {
      Dial_data = data;
    }
  ).fail(function (response) {
    ...
    if (counter_for_Nifty_15min == 0) {
      ...
      for (var i = 0; i < Array_1.length; i++) {
        if (i != Last_i_position_1) {
          vol_15min = parseFloat(Array_1[i][1]) + parseFloat(Array_1[i + 1][1]) + parseFloat(Array_1[i + 2][1]) + parseFloat(Array_1[i + 3][1]) + parseFloat(Array_1[i + 4][1])
          Volume_15min.push({
            x: parseFloat(Array_1[i][0]), // the date
...
function ohlc_and_Volume_15min_addition(Addition, length) {
  try {
...
$(document).ready(function () {
  ...
  a = $('#Candlestick_container').height(); console.log(a)
  b = $("#Top_Bar").height(); console.log(b)
  c = $("#Dials_Rows").height(); console.log(c)
  d = a - (c + $(".money_flux").height()); console.log("d = ", d)
  e = a - (c + $(".money_flux").height()) + 150; console.log("e = ", e)
  f = $(document).width(); console.log("width = ", f)
  ...
  // Apexchart Bar [Bottom Right Chart - HeatMap]
  var options = {
    series: [
      {
        data: HeatMap
    ...
    chart: {
      ...
      events: {
        dataPointSelection: (event, chartContext, dataPointIndex) => {
          ...
          let HeatMap_name = dataPointIndex['w']['globals']['categoryLabels'][temp];
          HeatMap_name = HeatMap_name.replace("-","_")
          HeatMap_name = HeatMap_name.replace("&","_")
          console.log(HeatMap_name);
          window.open(`https://www.tradingview.com/chart/?symbol=NSE%3A${HeatMap_name}`,'_blank');
        }
      }
    },
    dataLabels: {
  ...
  if ($(window).width() < 1425 && $(window).width() > 530) {
  ...
  if ($(window).width() > 1425) {
    $(".speedometerWrapper-G5piCoZi")
      .removeClass("small-G5piCoZi")
      .addClass("G5piCoZi");
  } else if ($(window).width() > 510 && $(window).width() < 1425) {
    $(".speedometerWrapper-G5piCoZi")
      .removeClass("G5piCoZi")
      .addClass("small-G5piCoZi");
  }
